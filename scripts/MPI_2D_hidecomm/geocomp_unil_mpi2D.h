...
void clean_cuda(){ 
    ...
    // CommOverlap
    cudaStreamDestroy(*streams); // to try
    //cudaDeviceReset();
}
...
#define update_sides(A,nx_A,ny_A) if (istep==0){ cudaDeviceSynchronize(); } else if (istep==1){ \
                                  cudaStreamSynchronize(streams[0]); \
                                  if (neighbours(0,1) != MPI_PROC_NULL)    write_to_mpi_sendbuffer_01<<<grid_mpi0,block_mpi0,0,streams[0]>>>(A##_send_01_d, A##_d, nx_A, ny_A, nx); cudaStreamSynchronize(streams[0]); \
                                  ...
                                  cudaStreamSynchronize(streams[0]); \
                                  if (neighbours(0,0) != MPI_PROC_NULL)    write_to_mpi_sendbuffer_00<<<grid_mpi0,block_mpi0,0,streams[0]>>>(A##_send_00_d, A##_d, nx_A, ny_A, nx); cudaStreamSynchronize(streams[0]); \
                                  ...
                                  cudaStreamSynchronize(streams[0]); \
                                  if (neighbours(0,0) != MPI_PROC_NULL)   read_from_mpi_recvbuffer_00<<<grid_mpi0,block_mpi0,0,streams[0]>>>(A##_d, A##_recv_00_d, nx_A, ny_A); cudaStreamSynchronize(streams[0]); \
                                  if (neighbours(0,1) != MPI_PROC_NULL)   read_from_mpi_recvbuffer_01<<<grid_mpi0,block_mpi0,0,streams[0]>>>(A##_d, A##_recv_01_d, nx_A, ny_A); cudaStreamSynchronize(streams[0]); \
                                  if (neighbours(1,1) != MPI_PROC_NULL)    write_to_mpi_sendbuffer_11<<<grid_mpi1,block_mpi1,0,streams[0]>>>(A##_send_11_d, A##_d, nx_A, ny_A, ny); cudaStreamSynchronize(streams[0]); \
                                  ...
                                  cudaStreamSynchronize(streams[0]); \
                                  if (neighbours(1,0) != MPI_PROC_NULL)    write_to_mpi_sendbuffer_10<<<grid_mpi1,block_mpi1,0,streams[0]>>>(A##_send_10_d, A##_d, nx_A, ny_A, ny); cudaStreamSynchronize(streams[0]); \
                                  ...
                                  MPI_Waitall(reqnr,req,MPI_STATUSES_IGNORE);  reqnr=0;  for (int j=0; j<NREQS; j++){ req[j]=MPI_REQUEST_NULL; }; \
                                  cudaStreamSynchronize(streams[0]); \
                                  if (neighbours(1,0) != MPI_PROC_NULL)   read_from_mpi_recvbuffer_10<<<grid_mpi1,block_mpi1,0,streams[0]>>>(A##_d, A##_recv_10_d, nx_A, ny_A); cudaStreamSynchronize(streams[0]); \
                                  if (neighbours(1,1) != MPI_PROC_NULL)   read_from_mpi_recvbuffer_11<<<grid_mpi1,block_mpi1,0,streams[0]>>>(A##_d, A##_recv_11_d, nx_A, ny_A); cudaStreamSynchronize(streams[0]); }
