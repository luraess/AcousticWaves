...
__global__ void compute_V(DAT* Vx, DAT* Vy, DAT* Vz, DAT* P, DAT dt, const DAT rho, DAT dx, DAT dy, DAT dz, const int nx, const int ny, const int nz){
    int ix = blockIdx.x*blockDim.x + threadIdx.x; // thread ID, dimension x
    int iy = blockIdx.y*blockDim.y + threadIdx.y; // thread ID, dimension y
    int iz = blockIdx.z*blockDim.z + threadIdx.z; // thread ID, dimension z
...
int main(int argc, char *argv[]){
    ...
    for (it=0;it<nt;it++){
        ...
        compute_V<<<grid,block>>>(Vx_d, Vy_d, Vz_d, P_d, dt, rho, dx, dy, dz, nx, ny, nz);  cudaDeviceSynchronize();
        update_sides(Vx,nx+1,ny  ,nz  );
        update_sides(Vy,nx  ,ny+1,nz  );
        update_sides(Vz,nx  ,ny  ,nz+1);
    }//it
    tim("Performance", Nix*Niy*Niz*(nt-3)*8*PRECIS/(1e9)); // timer test
