...
void __set_up_parallelisation(int argc, char *argv[]){
    ...
    if (me==0){ printf("nprocs=%d,dims(1)=%d,dims(2)=%d,dims(3)=%d \n", nprocs,dims[0],dims[1],dims[2]); }
}
...
#define update_sides(A,nx_A,ny_A,nz_A) cudaDeviceSynchronize(); \
                                       if (neighbours(0,1) != MPI_PROC_NULL)    write_to_mpi_sendbuffer_01<<<grid_mpi0,block_mpi0>>>(A##_send_01_d, A##_d, nx_A, ny_A, nz_A, nx); cudaDeviceSynchronize(); \
                                       ...
                                       if (neighbours(0,0) != MPI_PROC_NULL)    write_to_mpi_sendbuffer_00<<<grid_mpi0,block_mpi0>>>(A##_send_00_d, A##_d, nx_A, ny_A, nz_A, nx); cudaDeviceSynchronize(); \
                                       ...
                                       cudaDeviceSynchronize(); \
                                       if (neighbours(0,0) != MPI_PROC_NULL)   read_from_mpi_recvbuffer_00<<<grid_mpi0,block_mpi0>>>(A##_d, A##_recv_00_d, nx_A, ny_A, nz_A); cudaDeviceSynchronize(); \
                                       if (neighbours(0,1) != MPI_PROC_NULL)   read_from_mpi_recvbuffer_01<<<grid_mpi0,block_mpi0>>>(A##_d, A##_recv_01_d, nx_A, ny_A, nz_A); cudaDeviceSynchronize(); \
                                       if (neighbours(1,1) != MPI_PROC_NULL)    write_to_mpi_sendbuffer_11<<<grid_mpi1,block_mpi1>>>(A##_send_11_d, A##_d, nx_A, ny_A, nz_A, ny); cudaDeviceSynchronize(); \
                                       ...
                                       cudaDeviceSynchronize(); \
                                       if (neighbours(1,0) != MPI_PROC_NULL)    write_to_mpi_sendbuffer_10<<<grid_mpi1,block_mpi1>>>(A##_send_10_d, A##_d, nx_A, ny_A, nz_A, ny); cudaDeviceSynchronize(); \
                                       ...
                                       MPI_Waitall(reqnr,req,MPI_STATUSES_IGNORE);  reqnr=0;  for (int j=0; j<NREQS; j++){ req[j]=MPI_REQUEST_NULL; }; \
                                       cudaDeviceSynchronize(); \
                                       if (neighbours(1,0) != MPI_PROC_NULL)   read_from_mpi_recvbuffer_10<<<grid_mpi1,block_mpi1>>>(A##_d, A##_recv_10_d, nx_A, ny_A, nz_A); cudaDeviceSynchronize(); \
                                       if (neighbours(1,1) != MPI_PROC_NULL)   read_from_mpi_recvbuffer_11<<<grid_mpi1,block_mpi1>>>(A##_d, A##_recv_11_d, nx_A, ny_A, nz_A); cudaDeviceSynchronize(); \
                                       if (neighbours(2,1) != MPI_PROC_NULL)    write_to_mpi_sendbuffer_21<<<grid_mpi2,block_mpi2>>>(A##_send_21_d, A##_d, nx_A, ny_A, nz_A, nz); cudaDeviceSynchronize(); \
                                       ...
                                       cudaDeviceSynchronize(); \
                                       if (neighbours(2,0) != MPI_PROC_NULL)    write_to_mpi_sendbuffer_20<<<grid_mpi2,block_mpi2>>>(A##_send_20_d, A##_d, nx_A, ny_A, nz_A, nz); cudaDeviceSynchronize(); \
                                       ...
                                       MPI_Waitall(reqnr,req,MPI_STATUSES_IGNORE);  reqnr=0;  for (int j=0; j<NREQS; j++){ req[j]=MPI_REQUEST_NULL; }; \
                                       cudaDeviceSynchronize(); \
                                       if (neighbours(2,0) != MPI_PROC_NULL)   read_from_mpi_recvbuffer_20<<<grid_mpi2,block_mpi2>>>(A##_d, A##_recv_20_d, nx_A, ny_A, nz_A); cudaDeviceSynchronize(); \
                                       if (neighbours(2,1) != MPI_PROC_NULL)   read_from_mpi_recvbuffer_21<<<grid_mpi2,block_mpi2>>>(A##_d, A##_recv_21_d, nx_A, ny_A, nz_A); cudaDeviceSynchronize();
